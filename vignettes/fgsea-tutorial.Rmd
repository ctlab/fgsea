...
`fgsea` is an R-package for fast preranked gene set enrichment analysis (GSEA). This package allows you to estimate arbitrarily low P-values for individual gene sets. P-value estimation is based on an adaptive multi-level split Monte-Carlo scheme. See the [preprint](https://www.biorxiv.org/content/10.1101/060012v2) for algorithmic details. 
...
Loading example pathways and gene-level statistics and setting seed:
```{r}
...
As you can see from the warning, `fgsea` has a default lower bound `eps=1e-10` for estimating P-values. If you need to estimate P-value more accurate, you can set the `eps` argument to zero in the `fgsea` function.
...
plotGseaTable(examplePathways[topPathwaysUp], exampleRanks, fgseaRes, 
              gseaParam = 0.5)
```
...
fgseaResMain[, leadingEdge := lapply(leadingEdge, mapIds, 
                                     x=org.Mm.eg.db, 
                                     keytype="ENTREZID", 
                                     column="SYMBOL")]
