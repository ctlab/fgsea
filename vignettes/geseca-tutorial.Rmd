...
expression matrix *E*.
For each pathway *p* a score is calculated as:

$$
S(p) = \mathrm {Var}(\sum_{g \in p} (E[g]-\mathrm{mean}(E[g]))).
$$

This score correspond to a variance explained by a particular gene set.
...
Then we let obtain the pathway list. Here we use Hallmarks
...
Apply an appropriate normalization (note that we are using 10000 genes, 
...
```{r}
brain <- RunPCA(brain, assay = "SCT", verbose = FALSE, rev.pca = TRUE)
E <- brain@reductions$pca@feature.loadings
dim(E)
```
...
library(fgsea)
set.seed(1)
gesecaRes <- geseca(pathways, E, minSize = 5, maxSize = 500, scale = FALSE)    
...
for (ppn in topPathways) {
    ...
    score <- colSums(brain@assays$SCT@scale.data[pp, ])
