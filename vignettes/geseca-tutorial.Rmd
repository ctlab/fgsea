...
```{r}
set.seed(1)
gesecaRes <- geseca(exprs(es), pathways, minSize = 15, maxSize = 500)   
...
```{r}
topPathways <- gesecaRes[order(pval), pathway] |> head(10)

plotGesecaTable(pathways[topPathways], E=exprs(es), gesecaRes, titles = es$title)
```
